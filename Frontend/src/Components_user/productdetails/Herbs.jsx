...
const detail=()=>{
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8080/cosdetherb/${id}`)
      .then(response => {
  ...
  const isLoggedIn = localStorage.getItem('logindata');
    ...
    const handleBookNow = () => {
      if (isLoggedIn) {const logindata = JSON.parse(localStorage.getItem('logindata'));
  ...
  const { price, name, image, description,city,cp} = herb;
  const cpp=cp;
            ...
            <div className='row'>
                ...
                <div className='col-6'>
                    <div className='row'>
                        ...
                        <div className='col-12'>
                            <span>medicinal plant grown in fabrable condition</span><br></br>
                            Manufaturar: <span>xyz</span><br></br><br></br>
                        </div>
