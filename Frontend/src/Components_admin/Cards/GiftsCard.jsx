...
const FertilizerCard = ({  gift}) => {

  
  const { price, name, image, manufacturer, type,description , available} =  gift;
  ...
  const handleToggleAvailable = async () => {
    try {
      ...
      await axios.patch(`http://localhost:8080/fertilizers/${gift._id}/toggleAvailablegifts`, { available: !isAvailable });
    ...
    <div className='col-md-4 col-sm-6 col-lg-3 adminpro'>
        <div className='adminoverall'>
            ...
            <div className='adminproduct-content'>
                ...
                <p className='type'>Discription:</p>
                <button onClick={() => handleOrderClick() }  className="btn btn-primary"  >order</button>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp; &nbsp;
                <button onClick={() => handleToggleAvailable()} className="btn btn- avl"> {isAvailable ? 'Mark Unavailable' : 'Mark Available'}</button>
            </div>
        </div>
    </div>
  );
