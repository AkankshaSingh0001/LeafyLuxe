...
const FertilizerCard = ({ fertilizer }) => {
  ...
  const { price, name, image, manufacturer, type, available } = fertilizer;
  ...
  const handleToggleAvailable = async () => {
    try {
      ...
      await axios.patch(`http://localhost:8080/fertilizers/${fertilizer._id}/toggleAvailable`, { available: !isAvailable });
