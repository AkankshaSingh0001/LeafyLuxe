...
const FertilizerCard = ({ accessory }) => {

  
  const {price, name, image, manufacturer,material, available} = accessory;
  ...
  //     await axios.patch(`http://localhost:8080/fertilizers/${fertilizer._id}/toggleAvailableacc`, { available: !isAvailable });
    ...
    <div className='col-md-4 col-sm-6 col-lg-3 adminpro'>
        <div className='adminoverall'>
            ...
            <div className='price'>Rs. {price}</div>
                ...
                <p className='type'>Type: {material}</p>
                <button onClick={() => handleOrderClick() } className="btn btn-primary">order</button>
                {/* <button onClick={() => handleToggleAvailable()} className="btn btn-primary"> {isAvailable ? 'Mark Unavailable' : 'Mark Available'}</button> */}
            </div>
        </div>
    </div>
  );
