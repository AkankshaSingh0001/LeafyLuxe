...
const HerbsCard=({herbs})=>{
  

    const handleDelete = async () => {
        try {
          await axios.delete(`http://localhost:8080/herbs/${herbs._id}`);
      ...
      const handleEditClick = () => {
        ...
        window.location.href = `/editherbs/${herbs._id}`;
      };


    const { price,name,image,manufacturer} = herbs;
    return (
        <>
